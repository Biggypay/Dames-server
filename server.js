...
const chifoumiRooms = new Map(); // NOUVEAU: Salles Chifoumi
...
function startChifoumiTurnTimer(croom, roomId) {
  ...
  croom.turnTimer = setTimeout(() => {
    ...
    if (hp1 || hp2) { 
       // Fallback au cas où le client ne gère pas son propre timeout correctement
       const missingSlot = hp1 ? 2 : 1;
...
app.get('/chifoumi', serveHTML('chifoumi.html')); // Ajout Route Chifoumi
...
app.get('/chifoumi.html', serveHTML('chifoumi.html')); // Fallback iframe

// -- REST API MATCHMAKING & AUTH (conservé tel quel) --
...
io.on('connection', (socket) => {
  ...
  socket.on('chifoumi_join', ({ room, player, supabaseId, name, bet, currency }) => {
    ...
    if (croom.players[1] && croom.players[2] && croom.status === 'waiting') {
      ...
      // Le client gère le startRound, mais on prépare le timer côté serveur
  ...
  socket.on('chifoumi_choice', ({ room, player, choice, isTimeout }) => {
    ...
    // Transmettre à l'adversaire pour déclencher l'animation d'attente/révélation
    socket.to(room).emit('chifoumi_opponent_choice', { choice });

    if (croom.choices[1] && croom.choices[2]) {
      // Les deux ont joué, le serveur arrête ses timers et laisse les clients animer
      clearChifoumiTurnTimers(croom);
    }
  });
  ...
  socket.on('chifoumi_round_result', ({ room, player, round, result, playerChoice, opponentChoice }) => {
    const croom = chifoumiRooms.get(room);
    if (croom && croom.status === 'playing' && player === 1) { // On prend le slot 1 comme source de vérité pour l'historique
  ...
  socket.on('disconnect', () => {
    ...
    // Logique de déconnexion DRY pour tous les jeux multijoueurs
...
server.listen(PORT, '0.0.0.0', () => {
  console.log(`✅ Serveur Nano Banana (ex-Mindspille) v5.1 démarré sur le port ${PORT}`);
