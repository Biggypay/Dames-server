...
//  Dames 10x10 + Tic-Tac-Toe + Quoridor + Penalty Shootout
//  Temps réel — Render.com — Node 20
//  v5.0 — Penalty Shootout Multijoueur Complet
// ============================================================
...
function startDamesTurnTimer(droom, roomId, playerSlot) {
  ...
  droom.turnTimer = setTimeout(() => {
    ...
    droom.graceTimer = setTimeout(() => {
      ...
      console.log(`🏳️  [dames] Grâce expirée slot${playerSlot} → forfait (inactivité) | room:${roomId}`);
...
function startTTTTurnTimer(troom, roomId, playerSlot) {
  ...
  troom.turnTimer = setTimeout(() => {
    ...
    troom.graceTimer = setTimeout(() => {
      ...
      if (troom.status === 'finished') return;
      const winnerSlot = playerSlot === 1 ? 2 : 1;
      console.log(`🏳️  [ttt] Grâce expirée slot${playerSlot} → forfait (inactivité) | room:${roomId}`);
...
function startQuoriTurnTimer(qroom, roomId, playerSlot) {
  ...
  qroom.turnTimer = setTimeout(() => {
    ...
    qroom.graceTimer = setTimeout(() => {
      ...
      if (qroom.status === 'finished') return;
      const winnerSlot = playerSlot === 1 ? 2 : 1;
      console.log(`🏳️  [quori] Grâce expirée slot${playerSlot} → forfait (inactivité) | room:${roomId}`);
...
// ── Penalty ───────────────────────────────────────────────
// Durée par manche : 15s de jeu + 30s de grâce
...
function startPenaltyTurnTimer(proom, roomId) {
  ...
  proom.turnTimer = setTimeout(() => {
    ...
    // Auto-résolution si un joueur a déjà joué mais pas l'autre
    const hasP1 = proom.choices[1] !== undefined;
    const hasP2 = proom.choices[2] !== undefined;

    if (hasP1 || hasP2) {
      // Si un seul joueur a joué, on résout maintenant (l'autre n'a pas joué = zone null)
      console.log(`⏰ [penalty] Manche ${proom.currentRound} — temps écoulé, résolution auto | room:${roomId}`);
      resolvePenaltyRound(proom, roomId);
      return;
    }

    // Personne n'a joué → période de grâce
    const graceNow = Date.now();
    proom.graceStartTime = graceNow;
    io.to(roomId).emit('penalty_turn_warning', { startTime: graceNow, duration: PENALTY_GRACE_DURATION });
    console.log(`⚠️  [penalty] Manche ${proom.currentRound} — temps écoulé, grâce ${PENALTY_GRACE_DURATION/1000}s | room:${roomId}`);
    ...
    proom.graceTimer = setTimeout(() => {
      ...
      if (!hp1 && !hp2) {
        // Les deux absents → annulation
        console.log(`🏳️  [penalty] Personne n'a joué → cancel | room:${roomId}`);
        ...
        const cp = {
          ...
          penalty,
          p1Id: proom.players[1]?.supabaseId,
          p2Id: proom.players[2]?.supabaseId,
          betAmount: bet, currency: proom.currency || 'HTG'
        };
        if (proom.players[1]?.socketId) {
          io.to(proom.players[1].socketId).emit('game:over', { ...cp, myResult: -penalty });
          io.to(proom.players[1].socketId).emit('game:result', { postMessage: { ...cp, myResult: -penalty } });
        }
        if (proom.players[2]?.socketId) {
          io.to(proom.players[2].socketId).emit('game:over', { ...cp, myResult: -penalty });
          io.to(proom.players[2].socketId).emit('game:result', { postMessage: { ...cp, myResult: -penalty } });
        }
        io.to(roomId).emit('game:result', { postMessage: cp });
      } else {
        // Un seul joueur a joué → résolution (l'autre a zone=null)
        console.log(`🏳️  [penalty] Grace expirée, résolution avec ${hp1?'P1':'P2'} seulement | room:${roomId}`);
        resolvePenaltyRound(proom, roomId);
      }
...
// ── Résolution d'une manche Penalty ───────────────────────
function resolvePenaltyRound(proom, roomId) {
  clearPenaltyTurnTimers(proom);
  if (proom.status === 'finished') return;

  const round = proom.currentRound;
  // Slot 1 tire aux manches impaires, slot 2 tire aux manches paires
  const shooterSlot = round % 2 === 1 ? 1 : 2;
  const keeperSlot  = shooterSlot === 1 ? 2 : 1;

  const shooterZone = proom.choices[shooterSlot]; // undefined si pas joué
  const keeperZone  = proom.choices[keeperSlot];  // undefined si pas joué

  // Cas : tireur n'a pas joué → gardien gagne automatiquement
  // Cas : gardien n'a pas joué → tireur marque automatiquement
  // Cas : les deux ont joué → comparaison normale
  let isGoal;
  if (shooterZone === undefined || shooterZone === null) {
    isGoal = false; // Pas de tir = pas de but
  } else if (keeperZone === undefined || keeperZone === null) {
    isGoal = true;  // Pas de gardien = but automatique
  } else {
    isGoal = (shooterZone !== keeperZone);
  }
  ...
  console.log(`⚽ [penalty] Manche ${round} | Tireur:S${shooterSlot}(z:${shooterZone ?? 'null'}) Gardien:S${keeperSlot}(z:${keeperZone ?? 'null'}) → ${isGoal ? 'BUT' : 'ARRÊT'} | Score:${proom.scores.p1}-${proom.scores.p2} | room:${roomId}`);
  ...
  io.to(roomId).emit('penalty_round_result', {
    ...
    shooterSlot,
    keeperSlot,
    scores:     { p1: proom.scores.p1, p2: proom.scores.p2 },
    nextRound,
    gameOver:   gameIsOver
  });
  ...
  if (!gameIsOver) {
    ...
    proom.choices      = {};
    proom.choicesReady = 0;
    setTimeout(() => {
      if (proom.status === 'playing') startPenaltyTurnTimer(proom, roomId);
    }, 3500);
  } else {
...
function notifyPenaltyRoomOver(proom, roomId, winnerSlot, reason = 'normal') {
  ...
  if (winnerSlot === 0) {
    ...
    if (p1?.socketId) {
      io.to(p1.socketId).emit('game:over', { ...base, result: 'draw', myResult: 0 });
      io.to(p1.socketId).emit('game:result', { postMessage: { ...base, result: 'draw' } });
    }
    if (p2?.socketId) {
      io.to(p2.socketId).emit('game:over', { ...base, result: 'draw', myResult: 0 });
      io.to(p2.socketId).emit('game:result', { postMessage: { ...base, result: 'draw' } });
    }
  ...
  const base = {
    ...
    winnerSlot,
    winnerSupabaseId: winP?.supabaseId,
    loserSupabaseId:  losP?.supabaseId,
    p1Id: p1?.supabaseId, p2Id: p2?.supabaseId,
  ...
  if (winP?.socketId) {
    io.to(winP.socketId).emit('game:over', { ...base, result: 'win',  myResult: +netGain });
    io.to(winP.socketId).emit('game:result', { postMessage: { ...base, result: 'win',  myResult: +netGain } });
  }
  if (losP?.socketId) {
    io.to(losP.socketId).emit('game:over', { ...base, result: 'loss', myResult: -bet });
    io.to(losP.socketId).emit('game:result', { postMessage: { ...base, result: 'loss', myResult: -bet } });
  }
  io.to(roomId).emit('game:over', { ...base, result: 'win', myResult: +netGain });
  io.to(roomId).emit('game:result', { postMessage: base });
  console.log(`🏆 Penalty | Slot${winnerSlot} gagne ${proom.scores.p1}-${proom.scores.p2} | room:${roomId} | mise:${bet} gain:${netGain} HTG (${reason})`);
}
...
function notifyDamesRoomOver(room, roomIdOrReason, winnerSlotOrRoomId, reason = 'normal') {
  let roomId, winnerSlot;
  if (typeof roomIdOrReason === 'string' && typeof winnerSlotOrRoomId === 'number') {
    roomId = roomIdOrReason; winnerSlot = winnerSlotOrRoomId;
  } else {
    winnerSlot = roomIdOrReason; reason = winnerSlotOrRoomId || reason; roomId = room.id;
  }
  if (room.status === 'finished') return;
  room.status = 'finished';
  ...
  io.to(roomId).emit('game:over', { ...base, result: 'win', myResult: +netGain });
  io.to(roomId).emit('game:result', { postMessage: base });
  console.log(`🏆 Dames Multi | Slot${winnerSlot} gagne | room:${roomId} | mise:${bet} pot:${totalPot} commission:${commission} gain:${netGain} HTG (${reason})`);
}
...
app.get('/health', (req, res) => res.json({
  ...
  penaltyRooms: penaltyRooms.size,
...
// ══════════════════════════════════════════════════════════
//  ROUTE PENALTY — sert le HTML + injecte room si nécessaire
// ══════════════════════════════════════════════════════════
app.get('/penalty', (req, res) => {
  ...
  // Si room absent mais p1Id+p2Id présents → générer le ROOM_ID côté serveur
...
(function(){
  ...
  if (!u.searchParams.get('room')) {
    u.searchParams.set('room', '${generatedRoom}');
...
// Route directe pour compatibilité iframe
app.get('/penalty_shootout.html', (req, res) => {
  const f = path.join(PUBLIC, 'penalty_shootout.html');
  if (!fs.existsSync(f)) return res.status(404).send('penalty_shootout.html introuvable');
  res.setHeader('Content-Type', 'text/html;charset=utf-8');
  res.sendFile(f);
});
...
io.on('connection', (socket) => {
  ...
  socket.on('ttt_join', ({ room, player, supabaseId, name, bet, currency, manches }) => {
    ...
    if (troom.status === 'playing' || troom.status === 'paused') {
      const p1 = troom.players[1], p2 = troom.players[2];
      const bothBack = p1?.connected && p2?.connected;
      if (bothBack && troom.disconnectTimer) { clearTimeout(troom.disconnectTimer); troom.disconnectTimer = null; troom.status = 'playing'; io.to(room).emit('ttt_game_resumed', { message: 'Les deux joueurs sont de retour. La partie reprend !' }); console.log(`▶️  TTT | Reprise | room:${room}`); }
  ...
  socket.on('quoridor_join', ({ room, player, supabaseId, name, bet, currency }) => {
    ...
    if (qroom.status === 'playing' || qroom.status === 'paused') {
      const p1 = qroom.players[1], p2 = qroom.players[2];
      const bothBack = p1?.connected && p2?.connected;
      if (bothBack && qroom.disconnectTimer) { clearTimeout(qroom.disconnectTimer); qroom.disconnectTimer = null; qroom.status = 'playing'; io.to(room).emit('quoridor_game_resumed', { message: 'Les deux joueurs sont de retour. La partie reprend !' }); console.log(`▶️  Quoridor | Reprise | room:${room}`); }
  ...
  socket.on('penalty_join', ({ room, player, supabaseId, name, bet, currency }) => {
    // Validation : room obligatoire
    if (!room) {
      console.error(`❌ [penalty] penalty_join sans room ! socket:${socket.id} player:${player} name:${name}`);
      socket.emit('penalty_error', {
        message: 'room_id_missing',
        detail: 'Le paramètre room est absent. Vérifiez que p1Id, p2Id et room sont bien passés en paramètres URL.'
      });
      return;
    }
    ...
    console.log(`⚽ Penalty | Joueur ${player} (${name}) | room:${room} | statut:${proom.status}`);
    ...
    if (proom.status === 'playing' || proom.status === 'paused') {
      ...
      if (bothBack && proom.disconnectTimer) {
        clearTimeout(proom.disconnectTimer);
        ...
        console.log(`▶️  Penalty | Reprise | room:${room}`);
        ...
        if (proom.players[otherSlot] && !proom.players[otherSlot].connected) {
          proom.disconnectTimer = setTimeout(() => {
            ...
            if (!p1b && !p2b) {
              proom.status = 'finished';
              const { bet: b } = calcFinancial(proom.betAmount);
              const penalty = Math.round(b * 0.05);
              const cp = {
                type: 'game_over', game: 'penalty', room,
                result: 'cancel', reason: 'both_disconnected',
                penalty,
                p1Id: proom.players[1]?.supabaseId,
                p2Id: proom.players[2]?.supabaseId,
                betAmount: b, currency: proom.currency || 'HTG'
              };
              ...
              const ws = p1b ? 1 : 2;
              proom.status = 'playing';
              notifyPenaltyRoomOver(proom, room, ws, 'forfeit');
            }
          }, 60000);
        }
      }
      ...
      const opponentName = player === 1
        ? (proom.players[2]?.name || 'Adversaire')
        : (proom.players[1]?.name || 'Adversaire');

      socket.emit('penalty_start', {
        room,
        yourSlot: player,
        opponentName,
        bet: proom.betAmount,
        currency: proom.currency,
        reconnected: true,
        gameState: {
          round: proom.currentRound,
          scores: proom.scores,
          phase: 'choosing'
        }
      });

      // Sync timer si une manche est en cours
      if (proom.status === 'playing') {
        const now = Date.now();
        if (proom.graceStartTime) {
          socket.emit('penalty_turn_sync', { serverTime: now, startTime: proom.graceStartTime, duration: PENALTY_GRACE_DURATION });
        } else if (proom.turnStartTime) {
          socket.emit('penalty_turn_sync', { serverTime: now, startTime: proom.turnStartTime, duration: PENALTY_TURN_DURATION });
        }
      }
      return;
    }

    // ── Nouvelle partie : attendre les deux joueurs ──
    if (proom.players[1] && proom.players[2] && proom.status === 'waiting') {
      proom.status = 'playing';
      const p1 = proom.players[1], p2 = proom.players[2];

      io.to(p1.socketId).emit('penalty_start', {
        room,
        yourSlot: 1,
      ...
      io.to(p2.socketId).emit('penalty_start', {
        room,
        yourSlot: 2,
      ...
      console.log(`✅ Penalty | ${p1.name} (Slot1) vs ${p2.name} (Slot2) | room:${room} | mise:${proom.betAmount} ${proom.currency}`);

      // Démarrer le timer de la première manche après le toast d'intro (2.5s)
      setTimeout(() => {
        if (proom.status === 'playing') startPenaltyTurnTimer(proom, room);
      }, 2800);

    } else if (proom.status === 'waiting') {
      console.log(`⏳ Penalty | Attente 2ème joueur | room:${room} | slot:${player} (${name})`);
    }
  });

  // ── Réception du choix d'un joueur ──
  socket.on('penalty_choice', ({ room, player, round, zone }) => {
    if (!room) return;
    const proom = penaltyRooms.get(room);
    if (!proom || proom.status !== 'playing') return;
    if (proom.currentRound !== round) {
      console.warn(`[penalty] Choix pour manche ${round} mais manche courante = ${proom.currentRound} | room:${room}`);
      return;
    }
    // Enregistrer le choix (zone = 0, 1 ou 2 — index de la zone)
    proom.choices[player] = zone;

    // Notifier l'adversaire que ce joueur a fait son choix (sans révéler la zone)
    socket.to(room).emit('penalty_choice_received', { player });

    console.log(`🎯 [penalty] Manche ${round} | Slot${player} a choisi zone ${zone} | room:${room}`);

    // Si les deux joueurs ont joué → résolution immédiate
    if (proom.choices[1] !== undefined && proom.choices[2] !== undefined) {
      resolvePenaltyRound(proom, room);
    }
  });

  // ── Fin de partie envoyée par le client (fallback) ──
  ...
  socket.on('disconnect', () => {
    ...
    for (const [roomId, proom] of penaltyRooms.entries()) {
      ...
      clearPenaltyTurnTimers(proom);
      console.log(`⚠️  [penalty] Joueur ${disconnectedSlot} déconnecté | room:${roomId}`);
      pauseAndWatch({
...
server.listen(PORT, '0.0.0.0', () => {
  console.log(`✅ Serveur Mindspille v5.0 démarré sur le port ${PORT}`);
  ...
  console.log(`   /penalty_shootout.html → route directe`);
