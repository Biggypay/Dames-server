...
app.post('/game/join',requireAuth,(req,res)=>{
  ...
  if(!game){
    // Premier joueur — créer la salle avec ce gameId
    game={
      id:gameId,
  ...
  const base={
    gameId,betAmount:game.betAmount,
  ...
  return res.json({
    status:'ready',gameId,youAre:myColor,
    opponent:myColor==='white'?bUser?.username:wUser?.username,
  });
});
