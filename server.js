...
//  v5.1 — Intégration complète Chifoumi
// ============================================================
...
const chifoumiRooms = new Map(); // Ajout Chifoumi
...
function startChifoumiTurnTimer(croom, roomId) {
  ...
  // Informer les clients que le tour commence (15s)
  ...
  croom.turnTimer = setTimeout(() => {
    ...
    // Début de la période de grâce de 60s
    const graceNow = Date.now();
    ...
    croom.graceTimer = setTimeout(() => {
      ...
      if (!hp1 && !hp2) {
        ...
        // Un seul joueur a joué -> il gagne par forfait de l'autre
        const winnerSlot = hp1 ? 1 : 2;
...
function notifyChifoumiRoomOver(room, roomId, winnerSlot, reason = 'normal') {
  ...
  if (winnerSlot === 0) {
    ...
    io.to(roomId).emit('game:result', { postMessage: base });
    console.log(`🤝 Chifoumi | Match nul | room:${roomId}`);
    return;
  }
  ...
  io.to(roomId).emit('game:over', { ...base, result: 'win', myResult: +netGain });
  io.to(roomId).emit('game:result', { postMessage: base });
  console.log(`🏆 Chifoumi | Slot${winnerSlot} gagne | room:${roomId} | mise:${bet} gain:${netGain} HTG (${reason})`);
}

// ══════════════════════════════════════════════════════════
//  ROUTES HTTP (Sécurisées avec les alias -online)
// ══════════════════════════════════════════════════════════
...
app.get('/game-online.html', serveHTML('game.html'));

// ── DAMES 3D MULTIJOUEUR ──
app.get('/dames', serveHTML('dames_multi.html'));
app.get('/dames.html', serveHTML('dames_multi.html'));
app.get('/dames-online.html', serveHTML('dames_multi.html'));

// ── TIC-TAC-TOE ──
app.get('/ttt', serveHTML('ttt_game.html'));
app.get('/ttt.html', serveHTML('ttt_game.html'));
app.get('/ttt-online.html', serveHTML('ttt_game.html'));

// ── QUORIDOR ──
app.get('/quoridor', serveHTML('quoridor_multi.html'));
app.get('/quoridor.html', serveHTML('quoridor_multi.html'));
app.get('/quoridor-online.html', serveHTML('quoridor_multi.html'));
...
io.on('connection', (socket) => {
  ...
  socket.on('penalty_join', ({ room, player, supabaseId, name, bet, currency }) => {
    if (!room) {
      socket.emit('penalty_error', { message: 'room_id_missing', detail: 'Le paramètre room est absent.' });
      return;
    }
    socket.join(room);
    let proom = penaltyRooms.get(room);
    if (!proom) {
      proom = { id: room, players: {}, status: 'waiting', betAmount: bet || 0, currency: currency || 'HTG', disconnectTimer: null, currentRound: 1, scores: { p1: 0, p2: 0 }, choices: {}, choicesReady: 0, turnTimer: null, graceTimer: null, turnStartTime: null, graceStartTime: null };
  ...
  socket.on('penalty_choice', ({ room, player, round, zone }) => {
    ...
    if (proom.currentRound !== round) return;
  ...
  socket.on('chifoumi_choice', ({ room, player, choice }) => {
    ...
    if (croom.choices[1] && croom.choices[2]) clearChifoumiTurnTimers(croom);
  });
  ...
  socket.on('disconnect', () => {
    ...
    for (const [roomId, croom] of chifoumiRooms.entries()) {
      ...
      clearChifoumiTurnTimers(croom);
      console.log(`⚠️  [chifoumi] Joueur ${disconnectedSlot} déconnecté | room:${roomId}`);
      pauseAndWatch({
        room: croom, roomId, gameName: 'chifoumi',
        getP1: () => croom.players[1],
        getP2: () => croom.players[2],
...
server.listen(PORT, '0.0.0.0', () => {
  console.log(`✅ Serveur Nano Banana v5.1 démarré sur le port ${PORT}`);
  ...
  console.log(`   /*-online.html         → Alias directs pour iframes configurés`);
