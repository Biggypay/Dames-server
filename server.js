...
function notifyGameOver(game,winner,reason='checkmate'){
  ...
  // Annuler timer de déconnexion si encore actif
...
io.on('connection',(socket)=>{
  ...
  socket.on('ttt:join', ({gameId, playerId}) => {
    ...
    if(!game){
      ...
      // Annuler timer de forfait si actif
      if(game.disconnectTimer){
        clearTimeout(game.disconnectTimer);
        game.disconnectTimer = null;
  ...
  socket.on('ttt:move', ({gameId, cell, symbol}) => {
    ...
    // Relayer à l'adversaire uniquement
    socket.to(gameId).emit('ttt:move', {cell, symbol});
  });

  // ── TTT : fin de manche (envoyé par joueur X) ─────────
  ...
  // ── TTT : fin de match (envoyé par joueur X) ──────────
  socket.on('ttt:match_over', ({gameId, winner, scores}) => {
    ...
    io.to(gameId).emit('ttt:over', payload);
    io.to(gameId).emit('game:over', payload);
    io.to(gameId).emit('game:result', {postMessage: payload});
    console.log('🏆 TTT match terminé:', gameId, winner || 'nul');
  });

  // ── RECONNEXION — annule le timer de forfait ──────────
  ...
  // ── DÉCONNEXION — forfait en 15s ──────────────────────
  socket.on('disconnect',()=>{
    const userId=socketUsers.get(socket.id);
    socketUsers.delete(socket.id);
    if(!userId) return;

    // Chercher si ce joueur est dans une partie active
    for(const [gameId,game] of games.entries()){
      ...
      console.log(`⚠️  ${disconnectedColor} déconnecté de ${gameId} — forfait dans 15s`);

      // Notifier l'adversaire IMMÉDIATEMENT
      ...
      // Timer 15s → forfait si pas de reconnexion
    ...
    // Chercher aussi dans les parties TTT
