...
io.on('connection',(socket)=>{
  ...
  socket.on('dames_join', ({ room, player, supabaseId, name, bet, currency }) => {
    ...
    // Mettre à jour le socketId du joueur (reconnexion ou première connexion)
    const wasAlreadyIn = !!droom.players[player];
    droom.players[player] = { socketId: socket.id, supabaseId, name, slot: player };

    if (bet && !droom.betAmount) droom.betAmount = bet;

    console.log(`♟️  Dames | Joueur ${player} (${name}) rejoint/reconnecte la salle ${room} | statut: ${droom.status}`);

    // ── CAS 1 : RECONNEXION — la partie était déjà en cours ──
    if (droom.status === 'playing') {
      if (droom.disconnectTimer) {
        clearTimeout(droom.disconnectTimer);
        droom.disconnectTimer = null;
        console.log(`✅ Dames | ${name} reconnecté — timer forfait annulé (${room})`);
        socket.to(room).emit('player:reconnected', { message: `${name} est de retour !` });
      }
      const p1 = droom.players[1];
      const p2 = droom.players[2];
      const opponentName = player === 1 ? (p2 ? p2.name : 'Adversaire') : (p1 ? p1.name : 'Adversaire');
      // Envoyer l'état sauvegardé du plateau pour restauration complète
  ...
  socket.on('disconnect',()=>{
    ...
    for(const [roomId, droom] of damesRooms.entries()){
      ...
      if(droom.disconnectTimer) continue;

      const winnerSlot = disconnectedSlot === 1 ? 2 : 1;
      const dcName = droom.players[disconnectedSlot]?.name || `Joueur ${disconnectedSlot}`;

      console.log(`⚠️  Dames Multi | Joueur ${disconnectedSlot} (${dcName}) déconnecté de ${roomId} — forfait dans 60s`);
      io.to(roomId).emit('player:disconnected', {
        slot:      disconnectedSlot,
        countdown: 60,
        message:   `${dcName} déconnecté — victoire dans 60 secondes`,
      });
      io.to(roomId).emit('dames_opponent_disconnected', {
        slot: disconnectedSlot,
        message: `${dcName} s'est déconnecté.`,
      });

      droom.disconnectTimer = setTimeout(() => {
        if(droom.status !== 'playing') return;
        console.log(`🏳️  Dames Multi | Forfait Joueur ${disconnectedSlot} — room ${roomId}`);
        notifyDamesRoomOver(droom, winnerSlot, 'forfeit');
      }, 60000);
      break;
    }
