...
//  Render.com — Node 20
//  v3.6 — Fix forfait timeout joueur connecté inactif
// ============================================================
...
droom.turnTimer = setTimeout(() => {
  ...
  droom.graceTimer = setTimeout(() => {
    ...
    if (droom.status === 'finished') return;
    // FIX : forfait indépendamment du statut de connexion
...
// FIX : signature étendue avec roomId explicite pour cohérence avec TTT/Quoridor
function notifyDamesRoomOver(room, roomIdOrReason, winnerSlotOrRoomId, reason = 'normal') {
// Gestion rétrocompatible des deux signatures :
// ancienne : notifyDamesRoomOver(room, winnerSlot, reason)
// nouvelle : notifyDamesRoomOver(room, roomId, winnerSlot, reason)
let roomId, winnerSlot;
if (typeof roomIdOrReason === 'string' && typeof winnerSlotOrRoomId === 'number') {
  roomId = roomIdOrReason;
  winnerSlot = winnerSlotOrRoomId;
} else {
  // ancienne signature : (room, winnerSlot, reason)
  winnerSlot = roomIdOrReason;
  reason = winnerSlotOrRoomId || reason;
  roomId = room.id;
}
if (room.status === 'finished') return;
room.status = 'finished';
...
if (droom) {
  ...
  if (droom.status === 'playing') {
    if (isComplete === false) {
      // FIX : séquence de prises en cours — redémarrer le timer du joueur ACTUEL
      // pour éviter que la partie reste figée si le client ne termine pas sa séquence
      startDamesTurnTimer(droom, room, player);
    } else {
      // Coup terminé — timer de l'adversaire
