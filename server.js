...
//  Temps réel — Node 20
//  v5.3 — Smart Routing & Auto-Correction de fichiers
// ============================================================
...
// ══════════════════════════════════════════════════════════
//  ROUTES HTTP (Sécurisées & Multi-Noms)
// ══════════════════════════════════════════════════════════
...
// Routeur intelligent qui cherche le fichier peu importe comment il a été nommé sur GitHub
...
app.get(['/penalty', '/penalty.html', '/penalty_shootout.html', '/penalty-online.html'], serveSmart(['penalty_shootout.html', 'penalty-online.html', 'penalty.html'], true));
  ...
  socket.on('penalty_join', ({ room, player, supabaseId, name, bet, currency }) => {
    ...
    if (proom.status === 'playing' || proom.status === 'paused') {
      ...
      socket.emit('penalty_start', {
        room, yourSlot: player, opponentName, bet: proom.betAmount, currency: proom.currency, reconnected: true,
...
server.listen(PORT, '0.0.0.0', () => {
  console.log(`✅ Serveur Nano Banana v5.3 démarré sur le port ${PORT}`);
