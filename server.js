...
io.on('connection',(socket)=>{
  ...
  socket.on('dames_join', ({ room, player, supabaseId, name, bet, currency }) => {
    ...
    if (droom.status === 'playing') {
      // Annuler le timer de forfait s'il tourne
      ...
      // Renvoyer dames_start au joueur reconnecté pour relancer l'affichage
  ...
  // Relayer un coup à l'adversaire
  socket.on('dames_move', ({ room, player, from, to }) => {
    if (!room) return;
    // Broadcast à TOUS dans la salle SAUF l'émetteur
