...
//  Dames 10x10 + Tic-Tac-Toe + Quoridor + Penalty + Chifoumi
//  Temps réel — Node 20
//  v5.4 — Fix route penalty_shootout.html
// ============================================================
...
function startPenaltyTurnTimer(proom, roomId) {
  ...
  proom.turnTimer = setTimeout(() => {
    ...
    proom.graceTimer = setTimeout(() => {
      ...
      if (proom.status === 'finished') return;
      const hp1g = proom.choices[1] !== undefined;
      const hp2g = proom.choices[2] !== undefined;
      if (!hp1g && !hp2g) {
        ...
        notifyPenaltyRoomOver(proom, roomId, hp1g ? 1 : 2, 'timeout');
      }
...
function resolvePenaltyRound(proom, roomId) {
  ...
  if (!gameIsOver) {
    proom.currentRound = nextRound; proom.choices = {};
...
function startChifoumiTurnTimer(croom, roomId) {
  ...
  croom.turnTimer = setTimeout(() => {
    ...
    croom.graceTimer = setTimeout(() => {
      ...
      if (croom.status === 'finished') return;
      const hp1g = croom.choices[1] !== undefined;
      const hp2g = croom.choices[2] !== undefined;
      if (!hp1g && !hp2g) {
        ...
        notifyChifoumiRoomOver(croom, roomId, hp1g ? 1 : 2, 'timeout');
      }
...
// ══════════════════════════════════════════════════════════
//  ROUTES HTTP
//  serveSmart : cherche chaque nom dans PUBLIC, prend le premier trouvé
// ══════════════════════════════════════════════════════════
...
const serveSmart = (possibleNames, injectRoom = false) => (req, res) => {
  ...
  if (!foundPath) {
    return res.status(404).send(`${possibleNames[0]} introuvable. Noms testés : ${possibleNames.join(', ')}`);
  }
...
// ── ROUTES — ordre de recherche avec underscore en premier ──
app.get(['/game', '/game.html', '/game-online.html'],
  serveSmart(['game.html', 'game-online.html']));

app.get(['/dames', '/dames_multi.html', '/dames.html', '/dames-online.html'],
  serveSmart(['dames_multi.html', 'dames.html', 'dames-online.html']));

app.get(['/ttt', '/ttt_game.html', '/ttt.html', '/ttt-online.html'],
  serveSmart(['ttt_game.html', 'ttt.html', 'ttt-online.html']));

app.get(['/quoridor', '/quoridor_multi.html', '/quoridor.html', '/quoridor-online.html'],
  serveSmart(['quoridor_multi.html', 'quoridor.html', 'quoridor-online.html']));

app.get(['/chifoumi', '/chifoumi-online.html', '/chifoumi.html'],
  serveSmart(['chifoumi-online.html', 'chifoumi.html']));

// ── PENALTY : penalty_shootout.html EN PREMIER ──
app.get(['/penalty', '/penalty_shootout.html', '/penalty-online.html', '/penalty.html'],
  serveSmart(['penalty_shootout.html', 'penalty-online.html', 'penalty.html'], true));
...
io.on('connection', (socket) => {
  ...
  // ── RECONNEXION GÉNÉRIQUE ──
  function handleReconnection(roomState, room, player, name, gameName, notifyOverFn, startPayloadFn, turnDur, graceDur) {
    ...
    socket.emit(`${gameName}_start`, startPayloadFn(opponentName));
    if (roomState.status === 'playing') {
      const now = Date.now();
      if (roomState.graceStartTime) socket.emit(`${gameName}_turn_sync`, { serverTime: now, startTime: roomState.graceStartTime, duration: graceDur || GRACE_DURATION });
      else if (roomState.turnStartTime) socket.emit(`${gameName}_turn_sync`, { serverTime: now, startTime: roomState.turnStartTime, duration: turnDur || TURN_DURATION });
    }
  }
  ...
  socket.on('dames_join', ({ room, player, supabaseId, name, bet, currency }) => {
    ...
    if (droom.status === 'playing' || droom.status === 'paused') {
      handleReconnection(droom, room, player, name, 'dames', notifyDamesRoomOver,
        (oppName) => ({ room, yourSlot: player, opponentName: oppName, bet: droom.betAmount, currency: droom.currency, reconnected: true, boardState: droom.boardState || null, currentPlayer: droom.currentPlayer !== undefined ? droom.currentPlayer : 0, lastMove: droom.lastMove || null }),
        TURN_DURATION, GRACE_DURATION);
      return;
    }
  ...
  socket.on('ttt_join', ({ room, player, supabaseId, name, bet, currency, manches }) => {
    ...
    if (troom.status === 'playing' || troom.status === 'paused') {
      handleReconnection(troom, room, player, name, 'ttt', notifyTTTRoomOver,
        (oppName) => ({ room, yourSlot: player, opponentName: oppName, bet: troom.betAmount, currency: troom.currency, reconnected: true, gameState: troom.gameState || null, totalManches: troom.totalManches }),
        TURN_DURATION, GRACE_DURATION);
      return;
    }
  ...
  socket.on('quoridor_join', ({ room, player, supabaseId, name, bet, currency }) => {
    ...
    if (qroom.status === 'playing' || qroom.status === 'paused') {
      handleReconnection(qroom, room, player, name, 'quoridor', notifyQuoriRoomOver,
        (oppName) => ({ room, yourSlot: player, opponentName: oppName, bet: qroom.betAmount, currency: qroom.currency, reconnected: true, gameState: qroom.gameState || null, currentSlot: qroom.currentSlot }),
        TURN_DURATION, GRACE_DURATION);
      return;
    }
  ...
  socket.on('penalty_join', ({ room, player, supabaseId, name, bet, currency }) => {
    ...
    if (!proom) { proom = { id: room, players: {}, status: 'waiting', betAmount: bet || 0, currency: currency || 'HTG', disconnectTimer: null, currentRound: 1, scores: { p1: 0, p2: 0 }, choices: {}, turnTimer: null, graceTimer: null, turnStartTime: null, graceStartTime: null }; penaltyRooms.set(room, proom); }
    ...
    if (proom.status === 'playing' || proom.status === 'paused') {
      handleReconnection(proom, room, player, name, 'penalty', notifyPenaltyRoomOver,
        (oppName) => ({ room, yourSlot: player, opponentName: oppName, bet: proom.betAmount, currency: proom.currency, reconnected: true, gameState: { round: proom.currentRound, scores: proom.scores, phase: 'choosing' } }),
        PENALTY_TURN_DURATION, PENALTY_GRACE_DURATION);
      return;
    }
  ...
  socket.on('chifoumi_join', ({ room, player, supabaseId, name, bet, currency }) => {
    ...
    if (!croom) { croom = { id: room, players: {}, status: 'waiting', betAmount: bet || 0, currency: currency || 'HTG', disconnectTimer: null, currentRound: 1, scores: [0, 0], choices: {}, history: [], turnTimer: null, graceTimer: null, turnStartTime: null, graceStartTime: null }; chifoumiRooms.set(room, croom); }
    ...
    if (croom.status === 'playing' || croom.status === 'paused') {
      handleReconnection(croom, room, player, name, 'chifoumi', notifyChifoumiRoomOver,
        (oppName) => ({ room, yourSlot: player, opponentName: oppName, bet: croom.betAmount, currency: croom.currency, reconnected: true, gameState: { scores: croom.scores, currentRound: croom.currentRound, history: croom.history } }),
        CHIFOUMI_TURN_DURATION, CHIFOUMI_GRACE_DURATION);
      return;
    }
  ...
  socket.on('chifoumi_choice', ({ room, player, choice }) => {
    ...
    if (croom.choices[1] !== undefined && croom.choices[2] !== undefined) clearChifoumiTurnTimers(croom);
  });

  socket.on('chifoumi_round_start', ({ room, player, round }) => {
    const croom = chifoumiRooms.get(room);
    if (croom && croom.status === 'playing') { croom.currentRound = round; croom.choices = {}; startChifoumiTurnTimer(croom, room); }
  });
  ...
  socket.on('disconnect', () => {
    ...
    const processDisconnect = (roomsMap, gameName, clearTimersFn, notifyOverFn) => {
      for (const [roomId, roomState] of roomsMap.entries()) {
        ...
        pauseAndWatch({ room: roomState, roomId, gameName, getP1: () => roomState.players[1], getP2: () => roomState.players[2], winFn: (winnerIsP1) => notifyOverFn(roomState, roomId, winnerIsP1 ? 1 : 2, 'forfeit') });
        return true;
      }
      return false;
    };
...
server.listen(PORT, '0.0.0.0', () => {
  console.log(`✅ Serveur Nano Banana v5.4 démarré sur le port ${PORT}`);
  ...
  console.log(`   /penalty               → Penalty Shootout (penalty_shootout.html)`);
  console.log(`   /chifoumi              → Chifoumi (chifoumi-online.html)`);
