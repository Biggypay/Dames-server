...
//  Temps réel — Node 20
//  v5.6 — Correction Broadcasts Fin de Partie & Forfaits
// ============================================================
  ...
  proom.turnTimer = setTimeout(() => {
    ...
    proom.graceTimer = setTimeout(() => {
      ...
      if (!hp1Grace && !hp2Grace) {
        ...
        // Envoi ciblé au lieu de broadast global
...
function resolvePenaltyRound(proom, roomId) {
  ...
  if (!gameIsOver) {
    ...
    // 🔥 DELAI DE 4 SECONDES POUR PERMETTRE A L'ANIMATION DU DERNIER TIR DE SE TERMINER
    setTimeout(() => {
        if (proom.status === 'finished') return;
...
// ── FIN DE PARTIES (Notifications exclusives, sans broadcast trompeur) ──
...
const serveSmart = (possibleNames, injectRoom = false) => (req, res) => {
  ...
  try {
    ...
    for (let name of possibleNames) {
      ...
      if (match) { foundPath = path.join(PUBLIC, match); break; }
    }
    if (!foundPath) return res.status(404).send("Fichier introuvable.");
  } catch(e) {
