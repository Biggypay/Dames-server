...
//  Render.com — Node 20
//  v3.1 — Timers de tour côté serveur
// ============================================================
...
const TURN_DURATION  = 30 * 1000; // 30 secondes pour jouer
const GRACE_DURATION = 60 * 1000; // 60 secondes de grâce

// Efface tous les timers de tour actifs d'une room
...
// Démarre le timer de tour pour un joueur (slot 1 ou 2)
// Phase 1 : 30s → emit dames_turn_start
// Phase 2 : +60s de grâce → emit dames_turn_warning
// Phase 3 : expiration grâce → forfait du joueur actif
function startDamesTurnTimer(droom, roomId, playerSlot) {
  ...
  // Notifier les deux clients du début du timer
  ...
  // Phase 1 : 30s sans coup → déclencher la grâce
  droom.turnTimer = setTimeout(() => {
    ...
    // Notifier les deux clients de la période de grâce
...
io.on('connection',(socket)=>{
  ...
  socket.on('dames_join', ({ room, player, supabaseId, name, bet, currency }) => {
    ...
    if (droom.status === 'playing' || droom.status === 'paused') {
      ...
      // Envoyer l'état complet + état du timer pour resynchronisation
      ...
      // Resynchroniser le timer côté client reconnecté
      if (droom.turnPlayer !== null && droom.status === 'playing') {
        const now = Date.now();
        if (droom.graceStartTime) {
          // On est en période de grâce
    ...
    if (droom.players[1] && droom.players[2] && droom.status === 'waiting') {
      ...
      // Démarrer le timer du joueur 1 (blancs commencent toujours)
      // Petit délai pour laisser les clients s'initialiser
      setTimeout(() => {
  ...
  // Coup joué — relayer + sauvegarder + démarrer le timer de l'adversaire
  socket.on('dames_move', ({ room, player, from, to, boardState, currentPlayer }) => {
    ...
    if (droom) {
      ...
      droom.currentPlayer = currentPlayer !== undefined ? currentPlayer : 0;
      droom.lastMove      = { from, to, player };

      // Démarrer le timer de l'adversaire immédiatement après le coup
      if (droom.status === 'playing') {
        const nextSlot = player === 1 ? 2 : 1;
  ...
  function startTTTTurnTimer(game, gameId, symbol) {
    ...
    game.turnTimer = setTimeout(() => {
      ...
      game.graceTimer = setTimeout(() => {
        ...
        // Déclencher la fin de match via ttt:match_over
  ...
  socket.on('ttt:join', ({gameId, playerId, betAmount}) => {
    ...
    if(!game){
      ...
      // Démarrer le timer du premier joueur (X commence) après 3s
      setTimeout(() => {
  ...
  socket.on('ttt:move', ({gameId, cell, symbol}) => {
    ...
    // Démarrer le timer de l'adversaire
  ...
  socket.on('ttt:round_end', ({gameId, scores, round, nextStarter, tieBreak}) => {
    ...
    // Démarrer le timer du joueur qui commence la nouvelle manche
    setTimeout(() => {
