...
//  v4.0 — Ajout Penalty Shootout (même architecture)
// ============================================================
...
const penaltyRooms = new Map();   // ← NOUVEAU
...
// ── Penalty ───────────────────────────────────────────────
const PENALTY_TURN_DURATION  = 15 * 1000;  // 15s pour choisir
const PENALTY_GRACE_DURATION = 30 * 1000;  // 30s de grâce
...
function startPenaltyTurnTimer(proom, roomId) {
  ...
  // Les deux joueurs ont le même timer (choix simultané)
  ...
  proom.turnTimer = setTimeout(() => {
    ...
    proom.graceTimer = setTimeout(() => {
      ...
      if (!hasP1 && !hasP2) {
        ...
        // Celui qui n'a pas joué perd par forfait
...
function resolvePenaltyRound(proom, roomId) {
  ...
  // Slot 1 tire sur manches impaires, slot 2 sur manches paires
  ...
  if (!gameIsOver) {
    ...
    // Démarrer le timer de la prochaine manche après l'animation client (3.5s)
    setTimeout(() => {
    ...
    if (winnerSlot === 0) {
      // Égalité — on termine quand même (pas de mort subite pour l'instant)
...
function notifyPenaltyRoomOver(proom, roomId, winnerSlot, reason = 'normal') {
  ...
  if (winnerSlot === 0) {
    // Égalité : remboursement (pas de commission)
...
app.get('/penalty',  (req, res) => { const f = path.join(PUBLIC, 'penalty_shootout.html');   if (fs.existsSync(f)) { res.setHeader('Content-Type','text/html;charset=utf-8'); res.send(fs.readFileSync(f,'utf8')); } else res.status(404).send('penalty_shootout.html introuvable'); });
...
io.on('connection', (socket) => {
  ...
  socket.on('penalty_join', ({ room, player, supabaseId, name, bet, currency }) => {
    ...
    // Nouvelle partie : les deux joueurs sont là
    if (proom.players[1] && proom.players[2] && proom.status === 'waiting') {
      ...
      // Démarrer le timer de la manche 1 après le toast (2.5s)
  ...
  socket.on('penalty_choice', ({ room, player, round, zone }) => {
    ...
    if (proom.currentRound !== round) return; // manche obsolète, ignorer

    proom.choices[player] = zone;
    // Informer l'adversaire que l'autre a soumis (sans révéler la zone)
