...
//  Render.com — Node 20
//  v3.3 — Fix synchronisation multijoueur + prises multiples
// ============================================================
...
io.on('connection', (socket) => {
  ...
  //
  //  FIX SYNCHRONISATION :
  //  - Le serveur ne redémarre le timer adverse QUE si
  //    isComplete === true (séquence de prise terminée).
  //  - boardState et currentPlayer sont toujours mis à jour.
  //  - nextPlayer (0-indexed côté client) est retransmis
  //    tel quel à l'adversaire pour éviter toute conversion.
  //  - lastMove utilise from du premier step de la séquence.
  // ══════════════════════════════════════════════════════
  socket.on('dames_move', ({ room, player, from, to, steps, boardState, nextPlayer, isComplete }) => {
    ...
    if (droom) {
      // Sauvegarder l'état du plateau pour reconnexions
      if (boardState) droom.boardState = boardState;

      // FIX : nextPlayer (0-indexed) → currentPlayer stocké en 0-indexed
      if (nextPlayer !== undefined) {
        droom.currentPlayer = nextPlayer;
      } else {
        droom.currentPlayer = player === 1 ? 1 : 0;
      }

      // FIX : nextSlot calculé proprement depuis nextPlayer
      const nextSlot = nextPlayer !== undefined ? nextPlayer + 1 : (player === 1 ? 2 : 1);

      // FIX : lastMove utilise le premier step (origine réelle du coup)
      ...
      // FIX : timer adverse uniquement quand la séquence est complète
    ...
    socket.to(room).emit('dames_move', {
      ...
      // FIX : nextPlayer retransmis en 0-indexed, identique à ce que
      // le client emetteur a envoyé — pas de conversion supplémentaire
  ...
  function startTTTTurnTimer(game, gameId, symbol) {
    ...
    io.to(gameId).emit('ttt:turn_start', { symbol, startTime: now, duration: TTT_TURN_DURATION });
    ...
    game.turnTimer = setTimeout(() => {
      ...
      io.to(gameId).emit('ttt:turn_warning', { symbol, startTime: graceNow, duration: TTT_GRACE_DURATION });
  ...
  socket.on('ttt:join', ({ gameId, playerId, betAmount }) => {
    ...
    if (!game) {
      ...
      setTimeout(() => { if (game.status === 'playing') startTTTTurnTimer(game, gameId, 'X'); }, 3000);
      ...
      if (game.turnSymbol && game.status === 'playing') {
        const now = Date.now();
        if (game.graceStartTime) socket.emit('ttt:turn_sync', { serverTime: now, turnSymbol: game.turnSymbol, graceStartTime: game.graceStartTime });
        else if (game.turnStartTime) socket.emit('ttt:turn_sync', { serverTime: now, turnSymbol: game.turnSymbol, turnStartTime: game.turnStartTime });
      }
    }
  });
  ...
  socket.on('ttt:round_end', ({ gameId, scores, round, nextStarter, tieBreak }) => {
    ...
    setTimeout(() => { if (game.status === 'playing') startTTTTurnTimer(game, gameId, nextStarter); }, 2000);
  });
  ...
  socket.on('ttt:match_over', ({ gameId, winner, scores }) => {
    ...
    if (winner && winPlayer && losPlayer) {
      io.to(winPlayer.socketId).emit('ttt:over',    { ...base, result: 'win',  myResult: +netGain });
      io.to(winPlayer.socketId).emit('game:over',   { ...base, result: 'win',  myResult: +netGain });
      io.to(winPlayer.socketId).emit('game:result', { postMessage: { ...base, result: 'win',  myResult: +netGain } });
      io.to(losPlayer.socketId).emit('ttt:over',    { ...base, result: 'loss', myResult: -bet });
      io.to(losPlayer.socketId).emit('game:over',   { ...base, result: 'loss', myResult: -bet });
      io.to(losPlayer.socketId).emit('game:result', { postMessage: { ...base, result: 'loss', myResult: -bet } });
    } else {
      io.to(gameId).emit('ttt:over',    { ...base, result: 'draw', myResult: 0 });
      io.to(gameId).emit('game:over',   { ...base, result: 'draw', myResult: 0 });
      io.to(gameId).emit('game:result', { postMessage: { ...base, result: 'draw', myResult: 0 } });
    }
  ...
  socket.on('disconnect', () => {
    ...
    for (const [roomId, droom] of damesRooms.entries()) {
      ...
      // FIX : on suspend aussi le timer de tour pendant la déconnexion
      clearDamesTurnTimers(droom);
    ...
    for (const [gameId, game] of tttGames.entries()) {
      ...
      const tttRoom = {
        ...
        currency:        'HTG',
        disconnectTimer: game.disconnectTimer,
      };
      ...
      const getP = (sym) => {
        const p = game.players.find(pl => pl.symbol === sym);
        if (!p) return null;
        const u = users.get(p.userId);
        return { socketId: p.socketId, supabaseId: u?.supabaseId || p.userId, connected: p.connected !== false };
      };
      ...
      pauseAndWatch({
        ...
        winFn: (winnerIsP1) => {
          ...
          if (winPlayer) { io.to(winPlayer.socketId).emit('ttt:over', { ...base, result: 'win',  myResult: +netGain }); io.to(winPlayer.socketId).emit('game:over', { ...base, result: 'win',  myResult: +netGain }); io.to(winPlayer.socketId).emit('game:result', { postMessage: { ...base, result: 'win',  myResult: +netGain } }); }
          if (losPlayer) { io.to(losPlayer.socketId).emit('ttt:over', { ...base, result: 'loss', myResult: -bet });     io.to(losPlayer.socketId).emit('game:over', { ...base, result: 'loss', myResult: -bet });     io.to(losPlayer.socketId).emit('game:result', { postMessage: { ...base, result: 'loss', myResult: -bet } }); }
        },
      });
      break;
    }
