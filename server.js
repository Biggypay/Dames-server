...
//  Render.com — Node 20
//  v3.0 — Dames multijoueur en ligne
// ============================================================
...
const damesRooms   = new Map(); // roomId → dames room state  ← NOUVEAU
...
//  Utilisé par tous les jeux : Dames, TTT, et tout futur jeu
//
//  room        : objet salle avec { status, players, betAmount,
//                currency, disconnectTimer, boardState... }
//  getP1/getP2 : fonctions () => { socketId, supabaseId, connected }
//  winFn       : function(winnerIsP1) appelée si un seul joueur revient
//  gameName    : 'dames' | 'ttt' | nom du futur jeu
//  roomId      : identifiant de la salle (pour les logs et émissions)
// ══════════════════════════════════════════════════════════
function pauseAndWatch({ room, roomId, gameName, getP1, getP2, winFn }) {
  if (room.disconnectTimer) return; // timer déjà actif
  ...
  room.disconnectTimer = setTimeout(() => {
    ...
    if (p1back && p2back) {
      // Les deux revenus — impossible normalement (timer annulé)
      room.status = 'playing';
      return;
    }

    if (!p1back && !p2back) {
      // ── LES DEUX ABSENTS → annulation + pénalité 5% chacun ──
      ...
      // ── UN SEUL JOUEUR PRÉSENT → victoire sans pénalité ──
      const winnerIsP1 = p1back;
      room.status = 'playing'; // requis avant appel winFn
      winFn(winnerIsP1);
...
// Reprendre une partie après reconnexion
...
function notifyDamesRoomOver(room, winnerSlot, reason='normal'){
  ...
  const p1 = room.players[1]; // slot 1 = blancs
  const p2 = room.players[2]; // slot 2 = rouges
  ...
  // Envoyer aux deux joueurs dans la room
  ...
  // Broadcast room (pour compatibilité iframe postMessage)
...
// Jeu de dames (ancienne version API REST)
...
// Jeu de dames MULTIJOUEUR (nouvelle version)
...
io.on('connection',(socket)=>{
  ...
  // ══════════════════════════════════════════════════════
  //  DAMES MULTIJOUEUR — événements Socket.io
  // ══════════════════════════════════════════════════════

  // Joueur rejoint la salle de dames
  socket.on('dames_join', ({ room, player, supabaseId, name, bet, currency }) => {
    ...
    if (!droom) {
      droom = {
        ...
        disconnectTimer: null, boardState: null, currentPlayer: 0, lastMove: null,
      };
      damesRooms.set(room, droom);
    }

    // Enregistrer le joueur et le marquer connecté
    ...
    // ── CAS 1 : RECONNEXION (playing ou paused) ──
    if (droom.status === 'playing' || droom.status === 'paused') {
      ...
      if (bothBack && droom.disconnectTimer) {
        // Les deux de retour → annuler timer, reprendre
        ...
        // Joueur revenu, relancer timer pour l'absent
        droom.status = 'playing';
        ...
        if (droom.players[otherSlot] && !droom.players[otherSlot].connected) {
          droom.disconnectTimer = setTimeout(() => {
            ...
            if (!p1b && !p2b) {
              ...
              console.log(`❌ Dames | Deux absents → pénalité ${penalty} HTG | room:${room}`);
            } else {
              const ws = p1b ? 1 : 2;
              droom.status = 'playing';
      ...
      // Notifier discrètement l'adversaire
      ...
      // Renvoyer l'état complet du jeu
      const opponentName = player === 1 ? (p2?.name||'Adversaire') : (p1?.name||'Adversaire');
    ...
    // ── CAS 2 : LES DEUX JOUEURS SONT LÀ POUR LA PREMIÈRE FOIS ──
  ...
  // Relayer un coup à l'adversaire ET sauvegarder l'état
  socket.on('dames_move', ({ room, player, from, to, boardState, currentPlayer }) => {
    if (!room) return;
    // Sauvegarder l'état du jeu dans la salle
    const droom = damesRooms.get(room);
    if (droom) {
      droom.boardState = boardState || null;     // tableau 10x10 sérialisé
      droom.currentPlayer = currentPlayer || 0;  // 0=blanc, 1=rouge
      droom.lastMove = { from, to, player };
    }
    // Broadcast à l'adversaire
    socket.to(room).emit('dames_move', { room, player, from, to });
  });

  // Résultat final envoyé par le client (après handleGameEnd côté HTML)
  socket.on('dames_result', (data) => {
    const droom = damesRooms.get(data.room);
    if (!droom) return;
    // Trouver le slot gagnant
  ...
  // ── RECONNEXION (jeu de dames API REST) ───────────────
  ...
  socket.on('disconnect',()=>{
    ...
    for(const [gameId, game] of tttGames.entries()){
      ...
      // Notifier l'adversaire discrètement
      socket.to(gameId).emit('player:disconnected', {
        color: me.symbol, countdown: 60,
        message: 'Adversaire déconnecté — victoire dans 60 secondes',
      });

      console.log(`⚠️  [ttt] ${me.symbol} déconnecté | room:${gameId}`);

      // Construire les objets p1/p2 compatibles avec pauseAndWatch
      const tttRoom = {
        status:          game.status,
        betAmount:       game.betAmount,
        currency:        'HTG',
      ...
      Object.defineProperty(tttRoom, 'disconnectTimer', {
        get: () => game.disconnectTimer,
        set: (v) => { game.disconnectTimer = v; },
        configurable: true,
      });
      Object.defineProperty(tttRoom, 'status', {
        get: () => game.status,
        set: (v) => { game.status = v; },
        configurable: true,
      });
      ...
      pauseAndWatch({
        ...
        winFn: (winnerIsP1) => {
          ...
          const { bet, totalPot, commission, netGain } = calcFinancial(game.betAmount);
          const base = {
            type:'game_over', gameId, game:'ttt',
            winner: ws==='X'?'player1':'player2', winnerSymbol: ws,
            winnerSupabaseId: wUser?.supabaseId||winPlayer?.userId,
            loserSupabaseId:  lUser?.supabaseId||losPlayer?.userId,
            betAmount:bet, totalPot, commission, netGain, reason:'forfeit',
          };
