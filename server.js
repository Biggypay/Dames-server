...
io.on('connection',(socket)=>{
  ...
  socket.on('dames_join', ({ room, player, supabaseId, name, bet, currency }) => {
    ...
    if (!droom) {
      ...
      droom = {
        id:               room,
        players:          {},   // { 1: {socketId, supabaseId, name}, 2: {...} }
        status:           'waiting',
        betAmount:        bet || 0,
    ...
    // Enregistrer / mettre à jour le joueur
    ...
    console.log(`♟️  Dames | Joueur ${player} (${name}) rejoint la salle ${room}`);

    // Reconnecter ?
    if (droom.status === 'playing' && droom.disconnectTimer) {
      clearTimeout(droom.disconnectTimer);
      droom.disconnectTimer = null;
      io.to(room).emit('player:reconnected', { message: `${name} est de retour !` });
      console.log(`✅ Dames | ${name} reconnecté à ${room} — timer annulé`);
      return;
    }
