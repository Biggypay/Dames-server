...
//  v4.1 — Fix Penalty Shootout room joining
// ============================================================
...
function startPenaltyTurnTimer(proom, roomId) {
  ...
  proom.turnTimer = setTimeout(() => {
    ...
    console.log(`⚠️  [penalty] Manche ${proom.currentRound} — temps écoulé, grâce 30s | room:${roomId}`);
    proom.graceTimer = setTimeout(() => {
      ...
      if (!hasP1 && !hasP2) {
        ...
        const forfeitSlot = !hasP1 ? 1 : 2;
        const winnerSlot  = forfeitSlot === 1 ? 2 : 1;
        console.log(`🏳️  [penalty] Slot${forfeitSlot} inactif → forfait | room:${roomId}`);
...
function resolvePenaltyRound(proom, roomId) {
  ...
  console.log(`⚽ [penalty] Manche ${round} | Tireur:Slot${shooterSlot}(zone:${shooterZone}) Gardien:Slot${keeperSlot}(zone:${keeperZone}) → ${isGoal ? 'BUT' : 'ARRÊT'} | room:${roomId}`);

  io.to(roomId).emit('penalty_round_result', {
    p1Zone: proom.choices[1],
    p2Zone: proom.choices[2],
    isGoal,
    shooterSlot,
    keeperSlot,
  ...
  if (!gameIsOver) {
    ...
    const winnerSlot = proom.scores.p1 > proom.scores.p2 ? 1 : proom.scores.p2 > proom.scores.p1 ? 2 : 0;
    if (winnerSlot === 0) {
...
// ══════════════════════════════════════════════════════════
//  ROUTE PENALTY — sert le HTML + passe les query params
// ══════════════════════════════════════════════════════════
app.get('/penalty', (req, res) => {
  ...
  // ── CORRECTION CLÉ : si room est absent mais p1Id+p2Id sont présents,
  //    générer le ROOM_ID côté serveur et l'injecter dans la page.
  //    Cela garantit que les deux joueurs partagent exactement le même room.
  const { room, p1Id, p2Id } = req.query;
  if (!room && p1Id && p2Id) {
    const ids = [p1Id, p2Id].sort();
    const generatedRoom = 'penalty-' + ids[0].slice(-8) + '-' + ids[1].slice(-8);
    // Injecter un script en tête qui force le bon ROOM_ID avant que le JS client s'exécute
...
// Route alternative /penalty_shootout.html pour compatibilité iframe direct
...
io.on('connection', (socket) => {
  ...
  socket.on('penalty_join', ({ room, player, supabaseId, name, bet, currency }) => {
    if (!room) {
      // ── CORRECTION : si room est absent, on ne peut pas jouer.
      // Envoyer une erreur explicite plutôt que de silencieusement ignorer.
      console.error(`❌ [penalty] penalty_join reçu SANS room ! socket:${socket.id} player:${player} name:${name}`);
      socket.emit('penalty_error', { message: 'room_id_missing', detail: 'Le paramètre room est absent de l\'URL. Vérifiez que p1Id, p2Id et room sont bien passés en paramètres.' });
      return;
    }
    ...
    if (proom.status === 'playing' || proom.status === 'paused') {
      ...
      if (bothBack && proom.disconnectTimer) {
        clearTimeout(proom.disconnectTimer); proom.disconnectTimer = null;
      ...
      socket.emit('penalty_start', {
        ...
        bet: proom.betAmount, currency: proom.currency,
        reconnected: true,
        gameState: { round: proom.currentRound, scores: proom.scores, phase: 'choosing' }
      });
    ...
    if (proom.players[1] && proom.players[2] && proom.status === 'waiting') {
      ...
      console.log(`✅ Penalty | ${p1.name} vs ${p2.name} | room:${room} | mise:${proom.betAmount} ${proom.currency}`);
      setTimeout(() => { if (proom.status === 'playing') startPenaltyTurnTimer(proom, room); }, 2500);
    } else if (proom.status === 'waiting') {
      // Premier joueur connecté, on attend le second
      console.log(`⏳ Penalty | En attente du 2ème joueur | room:${room} | slot:${player} (${name})`);
    }
  });
  ...
  socket.on('disconnect', () => {
    ...
    for (const [roomId, proom] of penaltyRooms.entries()) {
      ...
      pauseAndWatch({ room: proom, roomId, gameName: 'penalty', getP1: () => proom.players[1], getP2: () => proom.players[2], winFn: (winnerIsP1) => notifyPenaltyRoomOver(proom, roomId, winnerIsP1 ? 1 : 2, 'forfeit') });
      break;
    }
...
server.listen(PORT, '0.0.0.0', () => {
  ...
  console.log(`   /penalty          → Penalty Shootout HTML`);
