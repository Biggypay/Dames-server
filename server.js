...
//  Render.com — Node 20
//  v3.4 — TTT aligné sur client ttt_online (events ttt_*)
// ============================================================
...
const tttRooms    = new Map(); // nouveau — remplace tttGames, même pattern que damesRooms
...
// ══════════════════════════════════════════════════════════
//  TIMERS GÉNÉRIQUES — réutilisés par Dames ET TTT
// ══════════════════════════════════════════════════════════
...
function startTTTTurnTimer(troom, roomId, playerSlot) {
  ...
  // Même event que Dames mais préfixe ttt_
...
// ══════════════════════════════════════════════════════════
//  PAUSE / REPRISE / ANNULATION — inchangé
// ══════════════════════════════════════════════════════════
...
app.get('/health', (req, res) => res.json({
  ...
  games: games.size, damesRooms: damesRooms.size, tttRooms: tttRooms.size,
...
io.on('connection', (socket) => {
  ...
  // ════════════════════════════════════════════════════════
  //  TTT MULTIJOUEUR — même pattern que Dames, events ttt_*
  // ════════════════════════════════════════════════════════
  socket.on('ttt_join', ({ room, player, supabaseId, name, bet, currency, manches }) => {
    ...
    if (!troom) {
      troom = {
        ...
        // État du jeu pour reconnexion
        gameState: null,   // { board, currentPlayer, matchW, matchR, manchesDone, mancheResults, isTiebreaker }
    ...
    if (troom.status === 'playing' || troom.status === 'paused') {
      ...
      // Envoyer l'état complet pour restauration
      socket.emit('ttt_start', {
        room, yourSlot: player, opponentName,
        bet: troom.betAmount, currency: troom.currency,
        reconnected: true,
        gameState: troom.gameState || null,
        totalManches: troom.totalManches,
      });
    ...
    if (troom.players[1] && troom.players[2] && troom.status === 'waiting') {
      ...
      io.to(p1.socketId).emit('ttt_start', {
        room, yourSlot: 1, opponentName: p2.name,
        bet: troom.betAmount, currency: troom.currency,
        reconnected: false, totalManches: troom.totalManches,
      });
      io.to(p2.socketId).emit('ttt_start', {
  ...
  // ── TTT : le client signale la fin d'une manche ───────
  socket.on('ttt_manche_end', ({ room, player, winner, winLine, matchW, matchR, manchesDone, mancheResults, isTiebreaker, nextStarterPlayer }) => {
    ...
    if (!troom.gameState) troom.gameState = {};
    troom.gameState.matchW           = matchW || 0;
    troom.gameState.matchR           = matchR || 0;
    troom.gameState.manchesDone      = manchesDone || 0;
    troom.gameState.mancheResults    = mancheResults || [];
    ...
    // Relayer aux deux joueurs pour qu'ils puissent afficher le résultat
    ...
    // Redémarrer le timer après le délai d'animation (~4s)
    setTimeout(() => {
  ...
  // ── TTT : déconnexion adversaire signalée au client ───
  // (géré dans disconnect, mais on expose l'event pour que
  //  le client puisse l'écouter avec ttt_opponent_disconnected)
